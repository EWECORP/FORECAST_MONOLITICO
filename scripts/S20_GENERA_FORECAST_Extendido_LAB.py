...
[2025-09-22] ACTUALIZAR Nuevos esquemas segmentados por módulo.  PUBLIC --> supply_planning
"""
...
def extender_datos_forecast(algoritmo, name, id_proveedor):
    ...
    # Obtener Sites
    stores = pd.read_sql("SELECT code, name, id FROM supply_planning.spl_site", conn) # type: ignore
    stores = stores[pd.to_numeric(stores['code'], errors='coerce').notna()].copy()
    stores['code'] = stores['code'].astype(int)

    # Obtener Productos
    products = pd.read_sql("SELECT ext_code, description, id FROM supply_planning.spl_product", conn) # type: ignore
