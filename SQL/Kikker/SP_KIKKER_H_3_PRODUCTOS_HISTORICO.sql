...
/****** Object:  StoredProcedure [dbo].[SP_KIKKER_H_3_PRODUCTOS_HISTORICO]    Script Date: 19/06/2025 15:36:19 ******/
SET ANSI_NULLS ON
GO
...
--exec [dbo].[SP_KIKKER_H_3_PRODUCTOS_HISTORICO] '2022','07'
CREATE procedure [dbo].[SP_KIKKER_H_3_PRODUCTOS_HISTORICO] @anio char(4), @mes char(2)
AS
...
IF OBJECT_ID('tempdb..##TEMP_MERCADOLOGICO_KIKKER') IS NULL  
	...
	C_ARTICULO			decimal(5, 0) 	NULL DEFAULT 0,
	C_FAMILIA 			decimal(5, 0) 	NULL DEFAULT 0,
	C_RUBRO  			decimal(5, 0) 	NULL DEFAULT  0,
	C_SUBRUBRO_1  		decimal(5, 0) 	NULL DEFAULT  0,
	C_SUBRUBRO_2 		decimal(5, 0) 	NULL DEFAULT  0,
	C_SUBRUBRO_3		DECIMAL(5,0)	NULL DEFAULT 0,
	C_SUBRUBRO_4		DECIMAL(5,0)	NULL DEFAULT 0,
	N_FAMILIA     		CHAR(30)        NULL ,
	N_RUBRO      		CHAR(30)      	NULL ,
	N_SUBRUBRO_1     	CHAR(30)      	NULL ,
	N_SUBRUBRO_2     	CHAR(30)        NULL ,
	N_SUBRUBRO_3		CHAR(30)		NULL ,
	N_SUBRUBRO_4		CHAR(30)		NULL )
	...
	select 
	C_ARTICULO, avg(L_PRECIOS) as L_COSTO
	into ##costos
	...
	convert (varchar,COST.L_COSTO),--PRECO_COMPRA
	''
--	'', --CURVA_CLIENTE
--	'', --PESO
--	'' --VOLUME
	 from [DIARCOP001].[DiarcoP].dbo.t050_articulos art
		inner join [DIARCOP001].[DiarcoP].dbo.T051_ARTICULOS_SUCURSAL suc on suc.C_ARTICULO=art.C_ARTICULO
		inner join [DIARCOP001].[DiarcoP].dbo.T052_ARTICULOS_PROVEEDOR prov on art.C_PROVEEDOR_PRIMARIO=prov.C_PROVEEDOR and art.C_ARTICULO=prov.C_ARTICULO
		inner join ##costos cost on ART.C_ARTICULO=cost.C_ARTICULO --and cost.C_PROVEEDOR=art.C_PROVEEDOR_PRIMARIO 
		inner join [DIARCOP001].[DiarcoP].dbo.T001_TABLA_CODIGO cod on art.c_UNIDAD_MEDIDA=cod.C_CODIGO_TABLA and c_tabla=79
		inner join ##TEMP_MERCADOLOGICO_KIKKER EST on EST.C_ARTICULO=art.C_ARTICULO and EST.C_RUBRO=art.C_RUBRO and EST.C_subrubro_1=art.C_subrubro_1 and EST.C_subrubro_2=art.C_subrubro_2
		where ART.C_ARTICULO in (select C_ARTICULO from ##surtido)
	group by
