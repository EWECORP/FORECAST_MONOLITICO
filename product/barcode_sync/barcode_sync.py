...
def run_barcode_sync():
    ...
    logging.info("creanado df_productos_connexa")
    df_productos_connexa = barcode_func.create_df_fnd_product() 
    logging.info(len(df_productos_connexa))

    # obtener el dataframe de los productos de Valkimia
    logging.info("creando df_productos_valkimia")
    df_productos_valkimia = barcode_func.create_df_articulos_valkimia() 
    logging.info(len(df_productos_valkimia))

    df_all = pd.DataFrame()

    logging.info("se recorre el dataframe df_productos_connexa")
    ...
    if (len(df_all)>0):
        ...
        logging.info(f"df_all = {df_all.shape}")
        ...
        logging.info(f"df_articulos_no_encontrados_en_valkimia = {df_articulos_no_encontrados_en_valkimia.shape}")
        ...
        if (len(df_articulos_no_encontrados_en_valkimia)>0):
            ...
            logging.info("se creo el archivo articulos_no_encontrados_en_valkimia")
        else:
            logging.info('no se creo el archivo articulos_no_encontrados_en_valkimia')
        ...
        logging.info(f"df_ean_validacion_false = {df_ean_validacion_false.shape}")
        ...
        if (len(df_ean_validacion_false)>0):
            ...
            logging.info("se creo el archivo ean_validacion_false")
        else:
            logging.info('no se creo el archivo ean_validacion_false')
        ...
        logging.info(f"df_ean_ocurrencias = {df_ean_ocurrencias.shape}")

        if (len(df_ean_ocurrencias)>0):
            barcode_func.dataFrameToCsv(nombre_archivo="ean_ocurrencias", 
                                            dataframe=df_ean_ocurrencias)
            logging.info("se creo el archivo ean_ocurrencias")
        else:
            logging.info('no se creo el archivo ean_ocurrencias')



    else:
        logging.info("df_all sin registros")

    logging.info(f"df_all = {df_all.shape}")
    barcode_func.dataFrameToCsv(nombre_archivo="df_all_procesado", dataframe=df_all)
    logging.info("Se genera el df_all_procesado ")
