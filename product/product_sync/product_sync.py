...
def run_product_sync():
    ...
    logging.info(f"Total de Productos recuperados en Diarco: {len(productos_diarco)}")   
    ...
    for articulo in productos_diarco:
        ...
        try:
            ...
            logging.error(reg, c_articulo, reg.size, e)
    ...
    logging.info(f"Cantidad de productos a insertar: {len(filas)}")
    ...
    logging.info(f"Cantidad de productos a modificar: {len(filas_upd)}")
    ...
    logging.info(f"Cantidad de productos no categorizados: {no_categorizados}")
    ...
    texto = f"Total de Productos recuperados en Diarco: {len(df_diarco)}"
    logging.info(texto)  

     # obtener el dataframe de los productos de Connexa
    df_connexa = product_func.create_df_fnd_product() 
    texto = f"Total de Productos recuperados en Connexa: {len(df_connexa)}"
    logging.info(texto)  
    ...
    texto = f"Total de Productos en Connexa para marcar de inactivos: {len(df_resultado)}"
    logging.info(texto)  
