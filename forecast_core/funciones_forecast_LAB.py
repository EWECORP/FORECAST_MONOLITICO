...
def actualizar_site_ids(df_forecast_ext, conn, name):
    ...
    """
    query = """
    SELECT code, name, id FROM supply_planning.spl_site
...
def get_execution(execution_id):
    ...
    try:
        ...
        query = """
            ...
            FROM supply_planning.spl_supply_forecast_execution
            WHERE id = %s
        """
...
def update_execution(execution_id, **kwargs):
    ...
    try:
        ...
        query = f"""
            UPDATE supply_planning.spl_supply_forecast_execution
            SET {set_clause}
            WHERE id = %s
        """
...
def get_full_parameters(supply_forecast_model_id, execution_id): # Parametros del Modelo(default_value) y de la Ejecución 
    ...
    try:
        ...
        query = """
            ...
            FROM supply_planning.spl_supply_forecast_model_parameter mp	
            FULL JOIN supply_planning.spl_supply_forecast_execution_parameter ep
...
def create_execution_parameter(supply_forecast_execution_id, supply_forecast_model_parameter_id, value):
    ...
    try:
        ...
        query = """
            INSERT INTO supply_planning.spl_supply_forecast_execution_parameter(
...
def get_execution_parameter(exec_param_id):
    ...
    try:
        ...
        query = """
            ...
            FROM supply_planning.spl_supply_forecast_execution_parameter
            WHERE id = %s
        """
...
def update_execution_parameter(exec_param_id, **kwargs):
    ...
    try:
        ...
        query = f"""
            UPDATE supply_planning.spl_supply_forecast_execution_parameter
            SET {set_clause}
            WHERE id = %s
        """
        cur.execute(query, tuple(values))
...
def delete_execution_parameter(exec_param_id):
    ...
    try:
        cur = conn.cursor()
        query = """
            DELETE FROM supply_planning.spl_supply_forecast_execution_parameter
            WHERE id = %s
        """
        cur.execute(query, (exec_param_id,))
        conn.commit()
...
def create_execution_execute(data_dict):
    ...
    try:
        ...
        insert_query = f"""
            INSERT INTO supply_planning.spl_supply_forecast_execution_execute(
                {', '.join(columns)}
            ) VALUES (
                {', '.join(['%s'] * len(columns))}
            )
...
def delete_execution_execute(exec_id):
    ...
    try:
        cur = conn.cursor()
        query = """
            DELETE FROM supply_planning.spl_supply_forecast_execution_execute
            WHERE id = %s
        """
...
def create_execution_execute_result(expected_demand, product_id, site_id, supply_forecast_execution_execute_id,
    ...
    try:
        ...
        query = """
            INSERT INTO supply_planning.spl_supply_forecast_execution_execute_result (
...
def get_execution_execute_result(result_id):
    ...
    try:
        cur = conn.cursor()
        query = """
            SELECT * FROM supply_planning.spl_supply_forecast_execution_execute_result
            WHERE id = %s
        """
...
def update_execution_execute_result(result_id, **kwargs):
    ...
    try:
        ...
        query = f"""
            UPDATE supply_planning.spl_supply_forecast_execution_execute_result
            SET {set_clause}
            WHERE id = %s
        """
        cur.execute(query, tuple(values))
...
def delete_execution_execute_result(result_id):
    ...
    try:
        cur = conn.cursor()
        query = """
            DELETE FROM supply_planning.spl_supply_forecast_execution_execute_result
            WHERE id = %s
        """
        cur.execute(query, (result_id,))
        conn.commit()
